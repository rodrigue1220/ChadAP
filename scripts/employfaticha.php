...
*/
	...
	$detemploys = $connection->query("SELECT * FROM wfp_chd_personnel 
									WHERE rh_nopers LIKE '%$cle%' OR rh_lname LIKE '%$cle%' OR rh_fname LIKE '%$cle%' OR rh_duty LIKE '%$cle%' OR rh_titre LIKE '%$cle%'
									ORDER BY rh_lname
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(rh_id) AS id FROM wfp_chd_personnel WHERE  rh_nopers LIKE '%$cle%' OR rh_lname LIKE '%$cle%' OR rh_fname LIKE '%$cle%' OR rh_duty LIKE '%$cle%' OR rh_titre LIKE '%$cle%' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT rh_id AS ID FROM wfp_chd_personnel WHERE  rh_nopers LIKE '%$cle%' OR rh_lname LIKE '%$cle%' OR rh_fname LIKE '%$cle%' OR rh_duty LIKE '%$cle%' OR rh_titre LIKE '%$cle%'")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="employfaticha.php" method="get">
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucun Profil Employ&eacute; trouver avec ce mot cl&eacute;... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"employfaticha.php?cle=".$cle."&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>"); ?>
