...
*/
	...
	$nbrdir = $mysqli->query("SELECT COUNT(dir_id) AS ID FROM wfp_chd_dir WHERE dir_fullname LIKE '%$cle%' OR dir_duty LIKE '%$cle%' OR dir_tel1 LIKE '%$cle%' OR dir_tel2 LIKE '%$cle%' OR dir_tel3 LIKE '%$cle%' OR dir_titre LIKE '%$cle%' OR dir_ext LIKE '%$cle%' OR dir_mail LIKE '%$cle%' OR dir_unit LIKE '%$cle%' ")->fetch_array();
	$nbr	= $nbrdir['ID'];
	...
	$detdirs = $connection->query("SELECT * FROM wfp_chd_dir 
									WHERE dir_fullname LIKE '%$cle%' OR dir_duty LIKE '%$cle%' OR dir_tel1 LIKE '%$cle%' OR dir_tel2 LIKE '%$cle%' OR dir_tel3 LIKE '%$cle%' OR dir_titre LIKE '%$cle%' OR dir_ext LIKE '%$cle%' OR dir_mail LIKE '%$cle%' OR dir_unit LIKE '%$cle%' 
									ORDER BY dir_fullname 
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(dir_id) AS id FROM wfp_chd_dir WHERE dir_fullname LIKE '%$cle%' OR dir_duty LIKE '%$cle%' OR dir_tel1 LIKE '%$cle%' OR dir_tel2 LIKE '%$cle%' OR dir_tel3 LIKE '%$cle%' OR dir_titre LIKE '%$cle%' OR dir_ext LIKE '%$cle%' OR dir_mail LIKE '%$cle%' OR dir_unit LIKE '%$cle%' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-fw fa-search"></i> R&eacute;sultats de la recherche......</h1>
	...
	<?php
		...
		$exis = $mysqli->query("SELECT dir_id AS ID FROM wfp_chd_dir WHERE dir_fullname LIKE '%$cle%' OR dir_duty LIKE '%$cle%' OR dir_tel1 LIKE '%$cle%' OR dir_tel2 LIKE '%$cle%' OR dir_tel3 LIKE '%$cle%' OR dir_titre LIKE '%$cle%' OR dir_ext LIKE '%$cle%' OR dir_mail LIKE '%$cle%' OR dir_unit LIKE '%$cle%' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="dirfaticha.php" method="get">
	...
	<?php 
		echo ("Liste des Utilisateurs et Coordonn&eacute;es | TOTAL : <b>".$allRecords."</b> | mot-cl&eacute; : <b>".$cle."</b> | Page : <b>".$page."</b>");		
	?>
		...
		<ul class="pagination">
			<li class="page-item <?php if($page <= 1){ echo 'disabled'; } ?>">
                <a class="page-link btn2 btn-primary"
                   href="<?php if($page <= 1){ echo '#'; } else { echo "?motcle=".$cle."&page=" . $prev; } ?>"><i class="fas fa-fw fa-arrow-left"></i> Pr&eacute;c&eacute;dent</a>
            </li>

            <li class="page-item <?php if($page >= $totoalPages) { echo 'disabled'; } ?>">
                <a class="page-link btn2 btn-primary"
                   href="<?php if($page >= $totoalPages){ echo '#'; } else {echo "?motcle=".$cle."&page=". $next; } ?>"> Suivant <i class="fas fa-fw fa-arrow-right"></i></a>
            </li>
        </ul>
    </nav>
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucune Information trouv&eacute;e... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"dirfaticha.php?motcle=".$cle."&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>"); ?>
