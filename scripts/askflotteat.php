...
*/
	...
	$detcargos = $connection->query("SELECT * FROM wfp_chd_cargodem 
									WHERE cargodem_state='APPROUVE'
									ORDER BY cargodem_tonnecharge DESC 
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(cargodem_id) AS id FROM wfp_chd_cargodem WHERE cargodem_state='APPROUVE' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-fw fa-edit"></i> <i class="fas fa-fw fa-truck"></i> Assign. Camion</h1>
	...
	<?php
		...
		$exis = $mysqli->query("SELECT cargodem_id AS ID FROM wfp_chd_cargodem WHERE cargodem_state='APPROUVE' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="askflotteat.php" method="get">
	...
	<?php 
		echo ("<i class=\"fas fa-fw fa-list\"></i> Liste des Demandes Approuv&eacute;es: <b>".$allRecords."</b> | Page : <b>".$page."</b>");
	?>
	</div>
    ...
    <table class="table table-bordered mb-5">
        <thead>
            <tr class="table-primary">
				...
				<th scope="col">D&eacute;j&agrave; Assign&eacute;</th>
		...
		<tbody>
		  <?php foreach($detcargos as $detcargo): 
			...
			if ($detcargo['cargodem_tonnecharge']>0)
			{
				echo("<tr bgcolor=\"#FFDBC2\"><td>".$detcargo['cargodem_id']."</td>");
				echo("<td>".$detcargo['cargodem_depart']."</td>");
				echo("<td>".$detcargo['cargodem_desti']."</td>");
				echo("<td>".$detcargo['cargodem_dure']."</td>");
				echo("<td>".$detcargo['cargodem_nat']." / ".$detcargo['cargodem_tonne']." T / ".$detcargo['cargodem_vol']." m3</td>");
				echo("<td>".$detcargo['cargodem_tonnecharge']." T / ".$detcargo['cargodem_volcharge']." m3</td>");
				echo("<td>".$detcargo['cargodem_dem']."</td>");
				echo("<td><a href=\"cargoassignrest.php?ide=".$detcargo['cargodem_id']."&page=".$page."\" class=\"d-none d-sm-inline-block btn btn-sm btn-info btn-circle shadow-sm\" title=\"Assigner\"><i class=\"fas fa-edit\"></i></a></td>"); 										
			}
			else
			{
				...
				echo("<td>".$detcargo['cargodem_tonnecharge']." T / ".$detcargo['cargodem_volcharge']." m3</td>");
				echo("<td>".$detcargo['cargodem_dem']."</td>");
				echo("<td><a href=\"cargoassign.php?ide=".$detcargo['cargodem_id']."&page=".$page."\" class=\"d-none d-sm-inline-block btn btn-sm btn-info btn-circle shadow-sm\" title=\"Assigner\"><i class=\"fas fa-edit\"></i></a></td>"); 										
			}
			echo '</tr>';
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucune Demande Approuv&eacute;es... </div>") ;	
			echo("</tbody></table>");
		}
	?>
