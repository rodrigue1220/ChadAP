                    ...
                    <h1 class="page-header">
						...
						<div align="right">
							<?php
								...
								if($nb['nb']!=0)
								{
									echo ("<button type=\"button\" class=\"btn btn-success btn2\" onclick=\"document.location='vadjoummahaskft.php'\" title=\"Confirmer Demande de Cong&eacute;s\"><i class=\"fa fa-check\" fa-fw></i> Confirmer Demande</button>");
								}
								else
								{
									$nb1 = $mysqli->query("SELECT COUNT(*) AS nb FROM wfp_chd_rqdjoummah WHERE lv_nopers='$nopers' AND (lv_state LIKE '%APPROUVE%' OR lv_state='' OR lv_state LIKE 'ATTENTE%') ")->fetch_array();		
									if($nb1['nb']==0)
									{
										echo ("<button type=\"button\" class=\"btn btn-primary btn2\" onclick=\"document.location='djoummahaskft.php'\" title=\"Nouvelle Demande de Cong&eacute;s\"><i class=\"fa fa-edit\" fa-fw></i> Nouvelle Demande</button>");
									}
									$nb2 = $mysqli->query("SELECT COUNT(*) AS nb FROM wfp_chd_rqdjoummah WHERE lv_nopers='$nopers' AND lv_state='APPROUVE2'")->fetch_array();		
									if($nb2['nb']!=0)
									{
										echo (" <button type=\"button\" class=\"btn btn-danger btn2\" onclick=\"document.location='djoummahaskconft.php'\" title=\"Confirmer Reprise de Cong&eacute;s\"><i class=\"fa fa-bell\" fa-fw></i> Confirmer Reprise</button>");
									}
							    }
									...
									while( $resultz = $requetez->fetch_assoc())
									{
										...
										if ($type == "AL")
										{
											if ($ldate <= $wakit)
											{
												...
												echo("<td>".$solde."</td>");
												echo("<td>".date("d.m.Y",strtotime($wakit))."</td></tr></tbody>");
											}
											else
											{
												...
												echo("<tbody><tr class=\"default\"><td>".$type."</td>");
												echo("<td>".$total."</td>");
												echo("<td>".date("d.m.Y",strtotime($ldate))."</td></tr></tbody>");
											}
										}
										else
										{
				...
				<div class="col-lg-6">
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
								...
								<form class="form-horizontal" name="formulaire" action="estimdjoummahft" method="post" >					
