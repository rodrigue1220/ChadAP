...
*/
	...
	$detrecstks = $connection->query("SELECT * FROM wfp_chd_logtransit 
									WHERE logt_destiwh='$wh' AND (logt_netdeliv='$cle' OR logt_desc LIKE '%$cle%' OR logt_batch LIKE '%$cle%' OR logt_wbs LIKE '%$cle%' OR logt_grantnum LIKE '%$cle%' OR logt_grantdesc LIKE '%$cle%')
									ORDER BY logt_destiwh
									LIMIT $paginationStart, $limit")->fetchAll();
	
	$cum	= $mysqli->query("SELECT SUM(logt_netdeliv) AS TOT FROM wfp_chd_logtransit WHERE logt_destiwh='$wh' AND (logt_netdeliv='$cle' OR logt_desc LIKE '%$cle%' OR logt_batch LIKE '%$cle%' OR logt_wbs LIKE '%$cle%' OR logt_grantnum LIKE '%$cle%' OR logt_grantdesc LIKE '%$cle%') ")->fetch_array();
	$cummul	= $cum["TOT"];
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(logt_id) AS id FROM wfp_chd_logtransit WHERE logt_destiwh='$wh' AND (logt_netdeliv='$cle' OR logt_desc LIKE '%$cle%' OR logt_batch LIKE '%$cle%' OR logt_wbs LIKE '%$cle%' OR logt_grantnum LIKE '%$cle%' OR logt_grantdesc LIKE '%$cle%') ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT logt_id AS ID FROM wfp_chd_logtransit WHERE logt_destiwh='$wh' AND (logt_netdeliv='$cle' OR logt_desc LIKE '%$cle%' OR logt_batch LIKE '%$cle%' OR logt_wbs LIKE '%$cle%' OR logt_grantnum LIKE '%$cle%' OR logt_grantdesc LIKE '%$cle%') ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<?php 
		echo ("Liste des enregistrements trouv&eacute;s <b>".$allRecords."</b> | WH: <a href=\"rechlogtrzreport.php?pwh=".$wh."\"><font color=\"#FF0000\">$wh</font></a> | Mot-cl&eacute;: <font color=\"#FF0000\">$cle</font> | Page : <b>".$page."</b>");
	?>
	</div>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"rechlogtrzreport2.php?pwh=".$wh."&motcle=".$cle."&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>");?>
