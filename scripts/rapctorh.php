...
*/
	...
	$ddln	= date('Y-m-01', strtotime('-1 month'));
	...
	$detdjmctos = $connection->query("SELECT * FROM wfp_chd_djmcto WHERE cto_statut!='ANNULE' AND cto_statut!='REJET' AND cto_deb>='$ddln' ORDER BY cto_id DESC LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(cto_id) AS id FROM wfp_chd_djmcto WHERE cto_statut!='ANNULE' AND cto_statut!='REJET' AND cto_deb>='$ddln' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-fw fa-hourglass-1"></i> Demandes CTO / H Supp.</h1>
	...
	<?php
		...
		$exis = $mysqli->query("SELECT cto_id AS ID FROM wfp_chd_djmcto WHERE cto_statut!='ANNULE' AND cto_statut!='REJET' AND cto_deb>='$ddln' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
    ...
    <div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="rapctorh.php" method="get">
		...
		<tbody>
		  ...
		  ?>
			<tr>
                ...
                <td><?php echo date("d.m.Y",strtotime($detdjmcto['cto_deb'])); ?></td>
				<td><?php echo $detdjmcto['cto_choix']; ?></td>
				<td><?php echo $detdjmcto['cto_dure']; ?></td>
				<td><?php echo $detdjmcto['cto_statut']; 
				echo("</td><td><a href=\"confrapcto.php?id=".$detdjmcto['cto_id']."&page=".$page."\" data-toggle=\"modal\" data-target=\"#confirmModal\" class=\"confirmModalLink d-none d-sm-inline-block btn btn-sm btn-warning btn-circle shadow-sm\" title=\"Annuler\"><i class=\"fa fa-reply\"></i></a></td>"); ?>										
			</tr>
          <?php endforeach; ?>
        </tbody>
    </table>
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucune demande de CTO &agrave; annuler...... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"rapctorh.php?&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>"); ?>
