...
*/
	...
	$detusers = $connection->query("SELECT * FROM wfp_chd_recapbil 
									WHERE rec_phone='$telephone' AND rec_mois LIKE '%2%' AND rec_status!='ANNULE'
									ORDER BY rec_id DESC
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(rec_id) AS id FROM wfp_chd_recapbil WHERE rec_phone='$telephone' AND rec_mois LIKE '%2%' AND rec_status!='ANNULE' ")->fetchAll();
	$allRecords = $sql[0]['id'];
	
	$sql2 = $connection->query("SELECT COUNT(rec_id) AS id FROM wfp_chd_recapbil WHERE rec_phone='$telephone' AND (rec_mois LIKE '%18' OR rec_mois LIKE '%19') AND rec_status!='ANNULE' ")->fetchAll();
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT rec_id AS ID FROM wfp_chd_recapbil WHERE rec_phone='$telephone' AND rec_mois LIKE '%2%' AND rec_status!='ANNULE' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="rechfactnum.php" method="get">
	...
	<?php 
		echo ("Facture (s) T&eacute;l&eacute;phonique | Num&eacute;ro: <font color=\"#FF0000\">$telephone</font> : <b>".$allRecords."</b> | Page : <b>".$page."</b> | <a href=\"rechfactnumarch.php?num=".$telephone."\"><b>ARCHIVES : ".$allRecords2."</b></a>");
	?>
	</div>
