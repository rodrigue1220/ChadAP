...
*/
	...
	// Get total records
	$sql = $connection->query("SELECT COUNT(logs_id) AS id FROM wfp_chd_logstock WHERE logs_wh='$wh' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT logs_id AS ID FROM wfp_chd_logstock WHERE logs_wh='$wh' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="rechlogstkreport.php" method="get">
			<input type="hidden" id="pwh" name="pwh" value="<?php echo $wh; ?>" />
	...
	<?php 
		echo ("Liste des enregistrements trouv&eacute;s <b>".$allRecords."</b> | WH: <font color=\"#FF0000\">$wh</font> | Tot. Tonnage: <font color=\"#FF0000\">$cumul</font> | Page : <b>".$page."</b>");
	?>
	</div>
    ...
    <table class="table table-bordered mb-5">
		...
		<tbody>
		  <?php foreach($detrecstks as $detrecstk): 
			...
			echo("<td><a href=\"logstkdet.php?id=".$detrecstk['logs_id']."\" class=\"d-none d-sm-inline-block btn btn-sm btn-info btn-circle shadow-sm\" title=\"D&eacute;tails\"><i class=\"fas fa-info\"></i></a></td>"); 										 	
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"rechlogstkreport.php?pwh=".$wh."&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>");?>
