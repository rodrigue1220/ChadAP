<?php
	...
	else
	{
		...
		{	
			$sql2 = "UPDATE wfp_chd_rqdjoummah SET lv_state='ATTENTE', lv_date='$date', lv_dateav='$firstday', lv_soldav='$soldav', lv_dateap='$lastday', lv_soldap='$soldap'
			WHERE lv_id='$id' ";					
			$requete2 = $mysqli->query($sql2) or die( $mysqli->connect_errno()) ;
			if( $requete2 )
			{
				...
				$fich	= $_SERVER['PHP_SELF'];
				$sql1 = "INSERT  INTO wfp_chd_tarikh (his_id, his_user, his_agent, his_page, his_date, his_lib1, his_lib2)
				VALUES ( '', '$pseudo', '$agent', '$fich', '$date', 'CONFIRMATION', 'Demande $id ATT SUP/OIC') ";
				$mysqli->query($sql1) or die ('Erreur '.$sql1.' '.$mysqli->error);
				include("inc/rissalaskdjmsup.php");
				header('Location:djoummah.php');		
			}
			else
			{
