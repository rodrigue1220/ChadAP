...
*/
  ...
  $users = $connection->query("SELECT * FROM user WHERE pseudo!='administrateur' AND state!='INACTIF' ORDER BY nom ASC LIMIT $paginationStart, $limit")->fetchAll();

  // Get total records
  $sql = $connection->query("SELECT count(id) AS id FROM user WHERE pseudo!='administrateur' AND state!='INACTIF'")->fetchAll();
  $allRecrods = $sql[0]['id'];
...
<link href="inc/iz-modal.css" rel="stylesheet" type="text/css" />
    ...
    <div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="indexlist.php" method="post">
		...
		<tbody>
		  <?php foreach($users as $user): ?>
            <tr>
                ...
                <td><?php echo $user['unite']; ?></td>
                <td><?php echo $user['indexid']; ?></td>
				<?php echo("<td><a href=\"indexmod.php?ide=".$user['id']."\" data-toggle=\"modal\" data-target=\"#confirmModal\" class=\"confirmModalLink btn btn-warning btn-sm d-sm-inline-block\" title=\"MODIFIER\"><i class=\"fa fa-edit\"></i></a></td>"); ?>
            </tr>
          <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <nav aria-label="Page navigation example">
		<ul class="pagination">
			...
			<?php for($i = $page+1; $i <= $page+4 && $page < $totoalPages-4; $i++ ): ?>
              <li class="page-item <?php if($page == $i ) {echo 'active'; } ?>">
                <a class="page-link" href="indexlist.php?page=<?= $i+1; ?>"> <?= $i+1; ?> </a>
              </li>
            <?php endfor; ?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		...
		<a href="#" class="btn btn-warning" id="confirmModalYes">Oui</a>
      </div>
