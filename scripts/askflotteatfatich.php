...
*/
	...
	$detcargos = $connection->query("SELECT * FROM wfp_chd_cargo 
									WHERE cargo_state='APPROUVE' AND (cargo_desti LIKE '%$cle%' OR cargo_depart LIKE '%$cle%' OR cargo_dem LIKE '%$cle%' OR cargo_vehi LIKE '%$cle%' OR cargo_chauf LIKE '%$cle%' OR cargo_tonne='$cle' OR cargo_nat='$cle')
									ORDER BY cargo_state
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(cargo_id) AS id FROM wfp_chd_cargo WHERE cargo_state='APPROUVE' AND (cargo_desti LIKE '%$cle%' OR cargo_depart LIKE '%$cle%' OR cargo_dem LIKE '%$cle%' OR cargo_vehi LIKE '%$cle%' OR cargo_chauf LIKE '%$cle%' OR cargo_tonne='$cle' OR cargo_nat='$cle')")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-fw fa-search"></i> <i class="fas fa-fw fa-edit"></i> Rechercher & Assign. Camion</h1>
	...
	<?php
		...
		$exis = $mysqli->query("SELECT cargo_id AS ID FROM wfp_chd_cargo WHERE cargo_state='APPROUVE' AND (cargo_desti LIKE '%$cle%' OR cargo_depart LIKE '%$cle%' OR cargo_dem LIKE '%$cle%' OR cargo_vehi LIKE '%$cle%' OR cargo_chauf LIKE '%$cle%' OR cargo_tonne='$cle' OR cargo_nat='$cle') ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	
	?>

	<form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" method="get" action="askflotteatfatich.php">
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="askflotteatfatich.php" method="get">
	...
	<?php 
		echo ("<i class=\"fas fa-fw fa-list\"></i> Liste des Demandes Approuv&eacute;es: <b>".$allRecords."</b> | mot-cl&eacute;: <b>".$cle."</b> | Page : <b>".$page."</b>");
	?>
	</div>
    ...
    <table class="table table-bordered mb-5">
		...
		<tbody>
		  <?php foreach($detcargos as $detcargo): 
			...
			echo("<td>".$detcargo['cargo_dem']."</td>");
			
			echo("<td><a href=\"cargoassign.php?ide=".$detcargo['cargo_id']."&page=".$page."\" class=\"d-none d-sm-inline-block btn btn-sm btn-info btn-circle shadow-sm\" title=\"Assigner\"><i class=\"fas fa-edit\"></i></a></td>"); 										
			echo '</tr>';
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucune Demande Approuv&eacute;es avec ce mot-cl&eacute;... </div>") ;	
			echo("</tbody></table>");
		}
	?>
