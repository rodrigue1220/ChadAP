...
*/
	...
	$detusers = $connection->query("SELECT * FROM user 
									WHERE pseudo!='administrateur' AND state='ACTIF' 
									ORDER BY nom
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(id) AS id FROM user WHERE pseudo!='administrateur' AND state='ACTIF'  ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	...
	<?php 
		echo ("Liste des Utilisateurs ACTIFS : <b>".$allRecords."</b> | Page : <b>".$page."</b>");
		echo(" | <a href=\"userlistin.php\">Utilisateurs INACTIFS : <strong>$iactif</strong></a>");	
		echo(" | <a href=\"userlistnvo.php\">Utilisateurs NOUVEAUX : <strong>$nvo</strong></a>");
	?>
	</div>
    ...
    <table class="table table-bordered mb-5">
		...
		<tbody>
		  <?php foreach($detusers as $detuser): 
			...
			echo("<td><a href=\"desacompte3.php?ide=".$detuser['id']."&choix=DESC&page=".$page."\" data-toggle=\"modal\" data-target=\"#confirmModal\" class=\"confirmModalLink d-none d-sm-inline-block btn btn-sm btn-danger btn-circle shadow-sm\" title=\"Desactiver\"><i class=\"fas fa-lock\"></i></a></td>"); 										
			echo '</tr>';
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucun Utilisateur ACTIF... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"userlist.php\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>");?>
