...
*/
	...
	$detdjoummahctos = $connection->query("SELECT * FROM wfp_chd_djmcto 
									WHERE cto_deb>='$date1' AND cto_deb<='$date2'
									ORDER BY cto_id DESC LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(cto_id) AS id FROM wfp_chd_djmcto WHERE cto_deb>='$date1' AND cto_deb<='$date2' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-fw fa-calendar"></i> Demandes HS | <font color="green"><i><?php echo date("d.m.Y",strtotime($_GET["datdeb"]));?></i></font> - <font color="green"><i><?php echo date("d.m.Y",strtotime($_GET["datfin"]));?></i></font></h1>
	...
	<?php
		...
		$exis = $mysqli->query("SELECT cto_id AS ID FROM wfp_chd_djmcto WHERE cto_deb>='$date1' AND cto_deb<='$date2' ")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
    ...
    <div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="rechctorh2d.php" method="get">
			<input type="hidden" id="datefin" name="datfin" value="<?php echo $date2; ?>" />
			<input type="hidden" id="datedeb" name="datdeb" value="<?php echo $date1; ?>" />
    ...
    <table class="table table-bordered mb-5">
        <thead>
            <tr class="table-primary">
				...
				<th scope="col">Heure d&eacute;but</th>
		...
		<tbody>
		  <?php foreach($detdjoummahctos as $detdjoummahcto): 
			...
			$nopers		= $detdjoummahcto['cto_index'];
			...
			echo("<tr class=\"default\"><td>".$detdjoummahcto['cto_id']."</td>");
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-info\">Aucune demande d'heures suppl&eacute;mentaires Enregistrée durant ces deux dates... </div>") ;	
			echo("</tbody></table>");
		}
	?>
