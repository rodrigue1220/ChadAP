...
include('connexion.php');
	...
	if ($profil == 'Normal')
	{
		$sql = "UPDATE user SET profil='' WHERE pseudo='$pseudo' ";		
		$requete = $mysqli->query($sql) or die( $mysqli->connect_errno()) ;
	}
	else
	{
		$sql = "UPDATE user SET profil='$profil' WHERE pseudo='$pseudo' ";		
		$requete = $mysqli->query($sql) or die( $mysqli->connect_errno()) ;
	}
	...
	if( $requete )
	{
		...
		$sql1 = "INSERT  INTO wfp_chd_tarikh (his_id, his_user, his_agent, his_page, his_date, his_lib1, his_lib2)
			VALUES ( '', '$pseudo', '$agent', '$fich', '$date', 'ACTION BASCULE PROFIL', '$pseudo $profil') ";
