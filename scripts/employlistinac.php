...
*/
	...
	$nbri 	= $mysqli->query("SELECT COUNT(rh_id) AS nb FROM wfp_chd_personnel WHERE rh_statut='ACTIF' ")->fetch_array();
	$trt	= $nbri['nb'];
	...
	$detemploys = $connection->query("SELECT * FROM wfp_chd_personnel 
									WHERE rh_statut='INACTIF' 
									ORDER BY rh_lname
									LIMIT $paginationStart, $limit")->fetchAll();
	
	// Get total records
	$sql = $connection->query("SELECT COUNT(rh_id) AS id FROM wfp_chd_personnel WHERE rh_statut='INACTIF' ")->fetchAll();
	$allRecords = $sql[0]['id'];
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT rh_id AS ID FROM wfp_chd_personnel WHERE rh_statut='INACTIF'")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
    ...
    <div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="employlistinac.php" method="get">
	...
	<?php 
		echo ("Liste des Employ&eacute;s INACTIFS : <b>".$allRecords."</b> | Page : <b>".$page."</b>");
		echo(" | <a href=\"employlist.php\">Employ&eacute;s ACTIFS : <strong>$trt</strong></a>");		
	?>
    ...
    <table class="table table-bordered mb-5">
		...
		<tbody>
		  <?php foreach($detemploys as $detemploy): 
			...
			echo("<td><a href=\"employmod.php?id=".$detemploy['rh_id']."&page=".$page."\" class=\"d-none d-sm-inline-block btn btn-sm btn-warning btn-circle shadow-sm\" title=\"Modifier\"><i class=\"fa fa-edit\"></i></a></td>"); 										
			echo("<td><a href=\"desacpte.php?id=".$detemploy['rh_id']."&cx=ACT&page=".$page."\" data-toggle=\"modal\" data-target=\"#confirmModal\" class=\"confirmModalLink d-none d-sm-inline-block btn btn-sm btn-success btn-circle shadow-sm\" title=\"Activer\"><i class=\"fas fa-unlock\"></i></a></td>"); 										
			echo '</tr>';
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucun Profil Employ&eacute; INACTIF... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"employlistinac.php?&page=".$page."\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>"); ?>
