...
*/
	...
	$detusers = $connection->query("SELECT * FROM user 
									WHERE pseudo!='administrateur' AND state='INACTIF' 
									ORDER BY nom
...
<!-- Page Heading -->
	...
	<?php
		...
		$exis = $mysqli->query("SELECT id AS ID FROM user WHERE pseudo!='administrateur' AND state='INACTIF'")->fetch_array();	  
		if($exis['ID'] != 0)
		{	
	...
	<div class="d-flex flex-row-reverse bd-highlight mb-3">
        <form action="userlistin.php" method="get">
	...
	<?php 
		echo ("Liste des Utilisateurs INACTIFS : <b>".$allRecords."</b> | Page : <b>".$page."</b>");
	...
	<?php
		...
		{
			echo("<div class=\"alert alert-danger\">Aucun Utilisateur INACTIF... </div>") ;	
			echo("</tbody></table>");
		}
	?>
  ...
  <!-- End of Page Wrapper -->
	...
	<div class="modal fade fond" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-footer">
		<?php echo("<a href=\"userlistin.php\" class=\"btn btn-danger\" id=\"confirmModalNo\">Non</a>");?>
