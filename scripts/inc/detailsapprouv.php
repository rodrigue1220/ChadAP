...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Demandes de Transport Approuv&eacute;es</h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
<!-- /.row -->			
			...
			<div class="panel-body">
                <div class="table-responsive">
                    <?php
						...
						$i=1;
						$exis = $mysqli->query("SELECT reqst_id AS ID FROM wfp_chd_request WHERE reqst_state='VALIDE' AND reqst_sens='$sofo'")->fetch_array();
						
						if($exis['ID'] != 0)
						{
							$sql = "SELECT * FROM wfp_chd_request WHERE reqst_state='VALIDE' AND reqst_sens='$sofo'" ;
							...
							while( $result = $requete->fetch_assoc()  )
							{
								...
								echo("<td><button type=\"button\" class=\"btn btn-default btn-circle\" onclick=\"document.location='impr1ask.php?id=".$result['reqst_id']."'\" title=\"EDITER\"><i class=\"fa fa-print\"></i></button></td>
									<td><button type=\"button\" class=\"btn btn-warning btn-circle\" onclick=\"document.location='annuleask.php?id=".$result['reqst_id']."'\" title=\"ANNULER\"><i class=\"fa fa-reply\"></i></button></td>
									<td><button type=\"button\" class=\"btn btn-primary btn-circle\" onclick=\"document.location='detail2reqst.php?id=".$result['reqst_id']."'\" title=\"DETAILS\"><i class=\"fa fa-list\"></i></button></td>");
