...
try {
	...
	$sql = "SELECT * FROM user WHERE profil='AdminSTOCK' " ;
	...
	while( $result = $requete->fetch_assoc()  )
	{
		...
		$body	.= 'Une Nouvelle Demande d\'&eacute;quipement APPROUVEE en ATTENTE de votre TRAITEMENT : <br /><br />';
		...
		$body	.= '<tr><td class="ligne">Approuv&eacute;e (OIC/ICT) par </td><td class="ligne"><b>'.$approverit.'</b></td></tr></table><br />';
		//$body	.= '<tr><td class="ligne">;Quantit&eacute; Approuv&eacute;e </td><td class="ligne"><b>'.$nmbr.'</b></td></tr>';
		//$body	.= '<tr><td class="ligne">Commentaires OIC/ICT </td><td class="ligne"><b>'.$comm.'</b></td></tr></table><br />';
		...
		$to = $messagerie;
		$mail->AddAddress($to);
		$mail->Subject  = "TRAITEMENT Demande Equipement";
