<?php
	...
	function getDateRep($daterep)
	{
		$retour	= $daterep;
		
		$drep	= mktime(0,0,0,substr($retour,5,2),substr($retour,8,2)+1,substr($retour,0,4));

		if (date("w", $drep)==6)
		{
			$drep = mktime(0,0,0,substr($retour,5,2),substr($retour,8,2)+3,substr($retour,0,4));
		}
		elseif (date("w", $drep)==0)
		{
			$drep = mktime(0,0,0,substr($retour,5,2),substr($retour,8,2)+2,substr($retour,0,4));
		}
		
		return $reprise = date("Y-m-d",$drep);
	}
	...
	function getCalcJours($date1,$date2)
	{
		...
		$nbjourcong = ($years*12*2.5)+($months*2.5);
